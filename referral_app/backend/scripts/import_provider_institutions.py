...
def parse_address(address_str: str) -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        if len(parts) != 2:
            logger.warning(f"Address doesn't have expected comma separator: {address_str}")
            return None
        ...
        if not match:
            logger.warning(f"Could not parse city/state/zip: {location_part}")
            return None
        ...
        try:
            ...
            for component, label in parsed:
                if label in ['OccupancyType', 'OccupancyIdentifier',
        ...
        logger.error(f"Error parsing address '{address_str}': {e}")
        return None
            ...
            async with session.begin():
                for record in data:
                    try:
                        ...
                        if 'id' not in record:
                            logger.error(f"Missing UUID for: {record.get('name', 'Unknown')}")
                            stats["errors"] += 1
                            continue

                        if 'name' not in record:
                            logger.error(f"Missing name for record with ID: {record['id']}")
                            stats["errors"] += 1
                            continue
                        ...
                        result = await session.execute(
                            select(ProviderInstitution).where(
                                ProviderInstitution.id == institution_uuid
                            )
                        )
                        ...
                        if record.get('address'):
                            ...
                            if not address_data:
                                logger.warning(
                                    f"Could not parse address for {record['name']}: "
                                    f"{record['address']}"
                                )

                        # Create Address record if we have valid address data
                        address_id = None
                        if address_data:
                        ...
                        logger.error(f"Error importing {record.get('name', 'Unknown')}: {e}")
                        stats["errors"] += 1
